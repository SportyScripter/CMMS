...
export const MessagesPage = () => {
  ...
  const fetchMessages = async () => {
    ...
    setError("");
    try {
      const endpoint = activeTab === "inbox" ? "/messages/inbox" : "/messages/outbox";
  ...
  const isMessageRead = (msg: Message) => {
    ...
    const recipientRecord = msg.recipients.find(r => r.recipient_id === currentUser.id);
  ...
  const filteredMessages = messages.filter((msg) => {
    ...
    const matchSender = `${msg.sender.name} ${msg.sender.lastname}`.toLowerCase().includes(searchLower);
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full p-12">
                ...
                <button onClick={fetchMessages} className="mt-4 text-blue-600 hover:underline text-sm font-medium">Spróbuj ponownie</button>
              </div>
            ) : filteredMessages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full p-12 text-gray-400">
                <Mail className="w-12 h-12 mb-4 opacity-20" />
                <p className="text-lg font-medium text-gray-600">Brak wiadomości</p>
              ...
              <div className="divide-y divide-gray-100">
                {filteredMessages.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      {/* Avatar placeholder */}
                      <div className={`w-10 h-10 rounded-full shrink-0 flex items-center justify-center text-sm font-bold shadow-sm border ${isRead ? "bg-gray-100 text-gray-500 border-gray-200" : "bg-blue-100 text-blue-700 border-blue-200"}`}>
                        {msg.sender.name.charAt(0)}{msg.sender.lastname.charAt(0)}
                      </div>
                      
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-baseline mb-1">
                          <h4 className={`text-sm truncate pr-4 ${isRead ? "font-medium text-gray-700" : "font-bold text-gray-900"}`}>
                            {activeTab === "inbox" ? `${msg.sender.name} ${msg.sender.lastname}` : `Do: ${msg.recipients.length} odbiorców`}
                          </h4>
                          <span className={`text-xs whitespace-nowrap ${isRead ? "text-gray-400" : "text-blue-600 font-semibold"}`}>
                            {formatDateTime(msg.sent_at)}
                          </span>
                        </div>
                        <p className={`text-sm truncate mb-1 ${isRead ? "font-normal text-gray-600" : "font-bold text-gray-900"}`}>
                          {msg.subject}
                        </p>
