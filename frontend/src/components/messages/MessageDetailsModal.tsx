...
export const MessageDetailsModal: React.FC<MessageDetailsModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && message && activeTab === "inbox") {
      const recipientRecord = message.recipients.find(r => r.recipient_id === currentUserId);
      if (recipientRecord && !recipientRecord.is_read) {
        api.patch(`/messages/${message.id}/read`)
          .then(() => onRead()) 
          .catch(err => console.error("Nie udało się oznaczyć jako przeczytane", err));
      }
    }
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-gray-50/50">
          <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm mb-4 space-y-4">
            <div>
              <h3 className="text-2xl font-bold text-gray-900">{message.subject}</h3>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 text-sm text-gray-600 border-t border-gray-100 pt-4">
              <div className="flex items-center">
                ...
                {message.sender.name} {message.sender.lastname} ({message.sender.role?.name || "Brak roli"})
              </div>
              <div className="flex items-center">
            ...
            {activeTab === "outbox" && (
              <div className="border-t border-gray-100 pt-4">
                <span className="font-semibold text-sm text-gray-600 block mb-2">Odbiorcy ({message.recipients.length}):</span>
                <div className="flex flex-wrap gap-2">
                  {message.recipients.map(r => (
                    <span key={r.recipient_id} className={`text-xs px-2 py-1 rounded-md border flex items-center ${r.is_read ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-gray-50 text-gray-600 border-gray-200'}`}>
        ...
        <div className="p-4 border-t border-gray-100 flex justify-end bg-white rounded-b-2xl">
          <button onClick={onClose} className="px-5 py-2 bg-gray-100 hover:bg-gray-200 text-gray-800 font-medium rounded-xl transition-colors">
            Zamknij
          </button>
        </div>
      </div>
