...
import { X, Edit2, Trash2, Key, Save, AlertCircle, Loader2 } from 'lucide-react';
...
export const UserDetailsModal: React.FC<UserDetailsModalProps> = ({ user, isOpen, onClose, onUpdated }) => {
  // --- UI & TAB STATES ---
  ...
  const [editRoleId, setEditRoleId] = useState<number | ''>(user?.role_id || '');
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/users/${user.id}/change-password`, { password: newPassword });
      onUpdated();
      setActiveTab('info');
  ...
  const handleDeleteUser = async () => {
    if (!window.confirm(`Are you sure you want to delete ${user.name} ${user.lastname}? This action cannot be undone.`)) {
      return;
    }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col animate-in fade-in duration-200">
        ...
        <div className="p-6">
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-100">
                <div>
                  ...
                  <p className="font-medium text-gray-900 mt-0.5">{user.name} {user.lastname}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase font-semibold">Numer SAP</p>
                  <p className="font-medium text-gray-900 mt-0.5">{user.sap_number}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase font-semibold">Rola</p>
                  <p className="font-medium text-blue-600 mt-0.5">{user.role?.name || 'Brak'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase font-semibold">Status</p>
                  <p className={`font-medium mt-0.5 ${user.is_active ? 'text-emerald-600' : 'text-red-500'}`}>
          ...
          {activeTab === 'edit' && (
            <form onSubmit={handleUpdateUser} className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div>
          ...
          {activeTab === 'password' && (
            <form onSubmit={handleChangePassword} className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="bg-amber-50 text-amber-800 text-sm p-3 rounded-lg border border-amber-200 mb-4 flex items-start">
                ...
                <p>Ustawiasz nowe hasło dla tego pracownika. Poprzednie przestanie działać po zapisaniu zmian.</p>
              </div>
