...
export const AddOrderModal: React.FC<AddOrderModalProps> = ({ isOpen, onClose, onUpdated }) => {
  // --- FORM STATES ---
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Błąd podczas tworzenia zlecenia lub checklisty.');
    } finally {
          ...
          <div className="p-6 overflow-y-auto flex-1">
            ...
            {isLoadingData ? (
              <div className="flex flex-col items-center justify-center py-10">
                ...
                <span className="text-sm text-gray-500">Pobieranie danych...</span>
              </div>
            ) : (
              <form id="add-order-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-6 gap-6">
                
                {/* LEFT COLUMN: MAIN FORM (takes 2 of 3 grid cols on large screens) */}
                <div className="lg:col-span-3 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Maszyna (opcjonalnie)</label>
                      <select 
                        value={machineId} 
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Przypisany technik (opcjonalnie)</label>
                      <select 
                        value={performedId} 
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Krótki opis zadania *</label>
                    <textarea 
                      required
                      rows={2}
                      value={description} 
                  ...
                  <div className="pt-2">
                    ...
                    >
                      <ListChecks className="w-4 h-4 mr-2" /> Dodaj elementy checklisty
                    </button>
                  </div>
                </div>
                ...
                <div className="lg:col-span-3 bg-gray-50 rounded-xl border max-w-full border-gray-100 p-4 flex flex-col min-h-[300px] max-h-[55vh]">
                  <h4 className="font-semibold text-gray-900 mb-1 flex items-center border-b border-gray-200 pb-2">
                    <ListChecks className="w-4 h-4 mr-2 text-indigo-600" /> Karta Wykonawcza
                  </h4>
                  ...
                  <div className="flex-1 mt-3 overflow-y-auto pr-1">
                    {checklistTasks.length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-center text-gray-400 p-4">
                        ...
                        <p className="text-sm">Brak dodanych kroków.<br/>Użyj przycisku obok, aby stworzyć listę zadań.</p>
                      </div>
                    ) : (
                      <ul className="space-y-2">
                        {checklistTasks.map((task, idx) => (
                          <li key={idx} className="flex items-start bg-white p-3 rounded-lg border border-gray-200 shadow-sm group">
